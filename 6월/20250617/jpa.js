...
// MyBatis는 mapper 설정을 통해 직접 DB와 연결한 후 개발자가 쿼리문을 통해 DB를 직접적으로 조작하는 기술로 즉시 DB에 반영된다.
// 반면 JPA는 영속성 컨텍스트(1차 캐시)를 통해 간접적으로 조작한 후 이를 DB와 동기화 하는 기술이기에 
// 영속성 컨텍스트가 조작된 데이터는 트랜젝션이 종료 되어야 DB에 저장된다.

/* JPA에서의 데이터 변경 흐름 */
// find() 혹은 JPQL로 데이터를 조회 → 영속성 컨텍스트에 저장
// Java 코드로 객체 필드를 수정 → DB에 아직 반영되지 않음!
// 트랜잭션 커밋 시점에 JPA가 변경된 필드만 감지해서 UPDATE 쿼리를 생성해 DB에 반영
// 즉, **"객체를 먼저 수정하고 나중에 DB에 반영하는 구조"**이다.
// JPA는 트랜잭션 안에서 영속 상태인 엔티티의 변화를 감지하고, 커밋할 때 자동으로 SQL을 생성해서 DB에 반영한다.
// 그렇기 때문에 개발자는 SQL문을 직접 작성하지 않아도 되며, 객체 조작에만 집중하면 된다는 점이 JPA의 큰 장점이다.
